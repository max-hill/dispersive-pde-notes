...
\begin{definition}[Well-Posedness]
  ...
  for any $u^{*}_{0}\in H_{x}^{s}(\R)$, there exists a time $T>0$ and an open
  ball $B\subset H_{x}^{s}$ containing $u_{0}^{*}$, and
  $X\subset C_{t}^{0}H_{x}^{s}([-T,T]\times \R)$ such that for each $u_{0}\in B$
  ...
  \begin{itemize}
    \item If, in addition $ X=C_{t}^{0}H_{x}^{s}[-T,T\times \R^{d}]$ from this
    $H_{x}^{s}$-well posedness is \textbf{unconditional}.
